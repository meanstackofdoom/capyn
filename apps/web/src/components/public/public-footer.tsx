...
const columns = [
  { title: "Explore", links: [["Mandate Studio", "/start"], ["Authority Lab", "/lab"], ["Product", "/product"], ["Security", "/security"], ["Pricing", "/pricing"], ["Developers", "/developers"], ["Documentation", "/docs"]] },
              ...
              <div className="mt-5 grid gap-3">
                {column.links.map(([label, href]) => (
                  <Link key={href} href={href} className="inline-flex items-center gap-1.5 text-sm font-semibold hover:text-authority">
                    {label}{(label === "Mandate Studio" || label === "Authority Lab") && <ArrowUpRight size={12} />}
                  </Link>
                ))}
              </div>
            </div>
