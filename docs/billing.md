...
Developer, Team and Business can all be selected during durable workspace onboarding. Team and Business are hosted-alpha plans, not self-serve live-execution offers. Public production work still requires a custom written scope covering the adapter, infrastructure and service boundary.
...
- onboarding plan intent that can create a Checkout session without changing entitlement before a signed webhook;
...
- SSO and SIEM export are Business entitlements but are not implemented in v0.4;
